    ...
    <div className="flex flex-col gap-6">
            ...
            {lists?.suggestions? 
                <div >
                    {lists?.suggestions.map((item:any, index:number) => (
                        <h2 className='text-sm cursor-pointer shadow-2xl' onClick={() =>{ setWhere(item.full_address), setLists([])}} 
        ...
        <div>
            ...
            {lists?.suggestions? 
                <div >
                    {lists?.suggestions.map((item:any, index:number) => (
                        <h2 className='text-sm cursor-pointer shadow-2xl' onClick={() =>{ setTo(item.full_address), setLists([])}} 
                         >{item.full_address}</h2>
                    ))}
                </div> : null
