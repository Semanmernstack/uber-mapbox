...
function Cars() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 p-3">
        {data?.map((d, index) => (
            <div key={index}
            className={`shadow-md gap-3 p-2 cursor-pointer rounded-md items-center bg-slate-300 justify-center  border-2 ${index===clickk?" bg-blue-500  hover:border-blue":null}`}
